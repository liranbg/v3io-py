...
class TestControlPlane(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_users(self):
        users = await v3io.controlplane.Users.list(self.client)
        ...
        self.assertEqual(self.test_username, users[0].attributes.username)
    ...
    async def test_create_update_delete_user(self):
        password = v3io.common.helpers.random_string(8) + "A1!"
        user = await self._create_dummy_user(password=password)

        other_client = v3io.controlplane.client.APIClient(endpoint=self.api_url)
        await other_client.login(username=user.attributes.username, password=password)
        me = await v3io.controlplane.Users.self(other_client)
        self.assertEqual(user.attributes.username, me.attributes.username)

        await me.update(self.client, attributes={"first_name": "liran2"})
        self.assertEqual("liran2", me.attributes.first_name)
        me = await v3io.controlplane.Users.get(self.client, me.id)
        self.assertEqual("liran2", me.attributes.first_name)

        # delete with admin client
        await me.delete(self.client)
    ...
    async def test_add_remove_from_user_group(self):
        ...
        user_group = await v3io.controlplane.UserGroups.create(self.client, name=group_name)
        self._resources_to_delete.append(user_group)
        self.assertEqual(group_name, user_group.attributes.name)
        self.assertNotEqual("", user_group.id)

        # create user
        user = await self._create_dummy_user()

        user_group = await user_group.get(self.client, user_group.id, include=["users"])
    ...
    async def _create_dummy_user(self, username=None, password=None):
        ...
        user = await v3io.controlplane.Users.create(self.client,
                                                    username=username,
                                                    password=password,
        ...
        self.assertEqual(username, user.attributes.username)
        return user

    async def _create_test_privilege_client(self):
        client = v3io.controlplane.client.APIClient(endpoint=self.api_url)
        await client.login(username=self.config["TEST_PRIVILEGED_USERNAME"], password=self.config["TEST_PRIVILEGED_PASSWORD"])
        return client
